...
import 'package:firebase_sync/src/reading/read_only_store_sync.dart';
import 'package:firebase_sync/src/reading/read_store_local_sync.dart';
...
void main() {
  ...
  late ReadStoreLocalSync<int> sut;

  setUp(() {
    reset(mockFirebaseStore);
    reset(mockStorage);

    when(() => mockStorage.isSync).thenReturn(true);

    sut = ReadOnlyStoreSync(
      firebaseStore: mockFirebaseStore,
      storage: mockStorage,
    );
  });

  test('length returns storage.length()', () {
    when(() => mockStorage.length()).thenReturn(42);

    expect(sut.length, 42);
  ...
  testData<Tuple2<int, bool>>(
    ...
    (fixture) {
      when(() => mockStorage.length()).thenReturn(fixture.item1);

      expect(sut.isEmpty, fixture.item2);
      verify(() => mockStorage.length());
    },
  );
  ...
  testData<Tuple2<int, bool>>(
    ...
    (fixture) {
      ...
      expect(sut.isNotEmpty, fixture.item2);
      verify(() => mockStorage.length());
    },
  );

  test('keys returns storage.keys()', () {
    const keys = ['a', 'b', 'c'];
    when(() => mockStorage.keys()).thenReturn(keys);

    expect(sut.keys, keys);
    verify(() => mockStorage.keys());
  });

  test('contains returns storage.contains()', () {
    const key = 'key';
    when(() => mockStorage.contains(any())).thenReturn(true);

    expect(sut.contains(key), isTrue);
    verify(() => mockStorage.contains(key));
  });

  test('clear calls storage.clear()', () {
    sut.clear();

    verify(() => mockStorage.clear());
  });

  test('asMap returns storage.entries()', () {
    const entries = <String, int>{
      'a': 1,
      'b': 2,
      'x': 3,
    };
    when(() => mockStorage.entries()).thenReturn(entries);

    expect(sut.asMap(), entries);
    verify(() => mockStorage.entries());
  });

  test('value returns storage.readEntry()', () {
    const key = 'key';
    when(() => mockStorage.readEntry(any())).thenReturn(42);

    expect(sut.value(key), 42);
  ...
  test('operator[] returns storage.readEntry()', () {
    const key = 'key';
    when(() => mockStorage.readEntry(any())).thenReturn(42);
